...
static void sortX(thread *queue) {
	thread t;
	thread prio_list;
	prio_list->Rel_Period_Deadline = NULL;
	for (t = readyQ; t != NULL; t = t->next){
		if (!prio_list->Rel_Period_Deadline){
			prio_list->Rel_Period_Deadline;
		} 
		if (prio_list->Rel_Period_Deadline < t->Rel_Period_Deadline) {
			prio_list = t;
		}
		...
		// 	highest_prio = t; 
		// } else {
		// 	if (highest_prio->Rel_Period_Deadline > t->Rel_Period_Deadline) {
		// 		highest_prio = t;
		// 	}
		}
	}
...
void respawn_periodic_tasks(void) {
	thread t;
	for (t = doneQ; t != NULL; t = t->next){
		if (ticks % t->Period_Deadline == 0){
			spawnWithDeadline(t->function, t->arg, t->Period_Deadline, t->Period_Deadline);
		} else {
			t->Rel_Period_Deadline -= 1;
		}
	}
}
